...
def balanced(m):
    ...
    left_balanced = is_planet(end(left(m))) or balanced(end(left(m)))
    right_balanced = is_planet(end(right(m))) or balanced(end(right(m)))
    return left_balanced and right_balanced and length(left(m)) * total_weight(end(left(m))) == length(right(m)) * total_weight(end(right(m)))
...
def has_path(t, word):
    ...
    if len(word) == 1 and label(t) == word[0]:
        return True
    elif label(t) != word[0] or (is_leaf(t) and len(word) > 1):
        return False
    else:
        for b in branches(t):
