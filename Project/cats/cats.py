...
def about(subject):
    ...
    "*** YOUR CODE HERE ***"
    def judge(sentence):
        sentence = lower(remove_punctuation(sentence))
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    for word in word_list:
        ...
        if diff < diff_min or (times == 0 and diff <= diff_min):
            diff_min = diff
            ans = word
            times += 1
    return ans
...
def time_per_word(words, timestamps_per_player):
    ...
    # BEGIN PROBLEM 9
    "*** YOUR CODE HERE ***"
    times = [[timestamps_per_player[j][i] - timestamps_per_player[j][i - 1] for i in range(1, len(timestamps_per_player[j]))] for j in range(len(timestamps_per_player))]
...
def fastest_words(match):
    ...
    for i in word_indices:
        ...
        for j in player_indices:
            if time(match, j, i) < time(match, player_min, i):
                player_min = j
        words[player_min].append(get_word(match, i))
    return words
    # END PROBLEM 10
