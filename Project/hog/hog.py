...
def max_scoring_num_rolls(dice=six_sided, samples_count=1000):
    ...
    ave_max = 0
    for i in range(10):
        temp = make_averaged(roll_dice, samples_count)(i + 1, dice)
        if temp > ave_max:
            result = i + 1
            ave_max = temp
    return result
...
def sus_strategy(score, opponent_score, threshold=11, num_rolls=6):
    ...
    # BEGIN PROBLEM 11
    score_plus = sus_update(0, score, opponent_score) - score
    if score_plus >= threshold:
        return 0
    else:
        return num_rolls
    # END PROBLEM 11
